...
gt_data = json.load(open('datasets/coco/annotations/panoptic_val2017.json','r'))
id2cat = {}
for gt in gt_data['annotations']:
    for g in gt['segments_info']:
        id2cat[g['id']] = g['category_id']
meta = MetadataCatalog.get('coco_2017_val_panoptic_separated').thing_dataset_id_to_contiguous_id
...
def pq_compute(gt_json_file, pred_json_file, gt_folder=None, pred_folder=None,
    ...
    for name, isthing, isunknown in metrics:
        results[name], per_class_results = pq_stat.pq_average(categories, isthing=isthing, isunknown=isunknown)
        if name == 'All':
...
class COCOOpeneval(COCOeval):
    ...
    def pr_evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        iouThrs = np.linspace(.0, 0.95, int(np.round((0.95 - .0) / .05)) + 1, endpoint=True)
    ...
    def pr_curve(self, p = None, name='test'):
        ...
        fps = []
        tps = []
        log_fps = [[[[[] for _ in range(M)] for _ in range(A)] for _ in range(K)] for _ in range(T)]
        log_tps = [[[[[] for _ in range(M)] for _ in range(A)] for _ in range(K)] for _ in range(T)]
        log_fps_cum_sum = [[[[[-1] for _ in range(M)] for _ in range(A)] for _ in range(K)] for _ in range(T)]
        log_tps_cum_sum = [[[[[-1] for _ in range(M)] for _ in range(A)] for _ in range(K)] for _ in range(T)]
        log_precisions = [[[[[-1] for _ in range(M)] for _ in range(A)] for _ in range(K)] for _ in range(T)]
        log_recalls = [[[[[-1] for _ in range(M)] for _ in range(A)] for _ in range(K)] for _ in range(T)]
        log_scores = [[[[-1] for _ in range(M)] for _ in range(A)] for _ in range(K)]
...
def get_gt(filename, panoptic_r, unseen_label_set=None):
    filename = filename.replace('.jpg', '.png').replace('/val', '/panoptic_val')
    segment_map, inst_info = panoptic_r
    gt = torch.tensor(rgb2id(np.asarray(Image.open(filename), dtype=np.uint32)).astype(int), device=segment_map.device)
    new_inst_info = []
