...
class StandardRPNHead_baseline(nn.Module):
    """
...
class RPN_baseline(nn.Module):
    ...
    def losses(
        ...
        if integral_sem_seg_targets is not None:
            ...
            normalizer = 0
            for labels, obj_logits, integral_sem_seg in zip(original_gt_labels, objectness_logits, integral_sem_seg_targets):
                valid_mask = labels >=0
                valid_bbox = anchors[valid_mask]
                valid_label = labels[valid_mask]
                valid_obj_logits = obj_logits[valid_mask]
                _, filtered_idx = add_unlabeled_class(valid_bbox, valid_label*2, integral_sem_seg, bg=0)
                objectness_loss += F.binary_cross_entropy_with_logits(valid_obj_logits[filtered_idx],
                            valid_label[filtered_idx].to(torch.float32), reduction="sum")
